...
def nothing_4_real_liner(time,x_pre, real_pre, name):
    plt.rcParams['figure.figsize'] = (16.0, 4.0)
    plt.rcParams['font.sans-serif'] = ['SimHei']  # 用来正常显示中文标签
    plt.rcParams['axes.unicode_minus'] = False  # 用来正常显示负号
    plt.xticks([])
    plt.plot(time,x_pre, color='blue', alpha=0.7, label='预测功率')
    plt.plot(time,real_pre, color='red', alpha=0.5, label='实际功率')
    plt.title('%s功率对比图' % name)
    plt.legend(loc='upper right')
    plt.show()
    return x_pre
...
def K_fold_weather(train_data, test_data, name, number, parm_list,bool):
    col_name = test_data.columns.values.tolist()
    col_name.remove('时间')
    col_name.remove('年')
    col_name.remove('月')
    col_name.remove('日')
    ...
    col_name.remove('白天')
    col_name.remove('id')
    train_col_name = col_name.copy()
    train_col_name.insert(0, '实际功率')
    X = train_data[train_data['白天'] == bool][train_col_name]
    y = test_data[test_data['白天'] == bool][col_name]
    max_depth = parm_list[0]
    learning_rate = parm_list[1]
    n_estimators = parm_list[2]
    param = {
    ...
    dtrain = xgb.DMatrix(
        X[col_name].values, label=X['实际功率'].values)
    dtest = xgb.DMatrix(y.values)
    cv_res = xgb.cv(param, dtrain, num_boost_round=1000, early_stopping_rounds=200, nfold=10, metrics='mae',
                    verbose_eval=True,
                    )
    bst = xgb.train(param, dtrain, num_boost_round=cv_res.shape[0])
    x_pre = bst.predict(xgb.DMatrix(X[col_name].values))
    real_pre = X['实际功率'].values
    score_data = pd.DataFrame()
    score_data['时间'] = train_data[train_data['白天'] == bool]['时间']
    score_data['实际功率'] = train_data[train_data['白天'] == bool]['实际功率']
    res = nothing_4_real_liner(score_data['时间'],x_pre, real_pre, name)
    score_data['预测功率'] = res
    score_data.to_csv(r'E:\国能竞赛\dataset\score.csv', index=False)
    res_score = score.score(number, r'E:\国能竞赛\dataset\score.csv')
    log_in(name, res_score, bool)
...
def lgb_Kfold(train_data, test_data, name, number, parm_list, bool):
    ...
    train_col_name.insert(0,'实际功率')
    # X = train_data[train_data['白天'] == bool][
    #     ['辐照度','风速','风向','温度','湿度','压强','月','日','时','分','实际功率']]
    # y = train_data['实际功率']
    # y = test_data[test_data['白天'] == bool][
    #     ['辐照度','风速','风向','温度','湿度','压强','月','日','时','分',]]
    X = train_data[train_data['白天'] == bool][train_col_name]
    y = test_data[test_data['白天'] == bool][col_name]
    max_depth = parm_list[0]
    learning_rate = parm_list[1]
    ...
    x_pre = bst.predict(X[col_name].values, num_iteration=bst.best_iteration)
    real_pre = X['实际功率'].values
    # res = nothing_4_real(x_pre, real_pre, name)
    score_data = pd.DataFrame()
    score_data['时间'] = train_data[train_data['白天'] == bool]['时间']
    score_data['实际功率'] = train_data[train_data['白天'] == bool]['实际功率']
    res = nothing_4_real_liner(train_data[train_data['白天'] == bool]['时间'],x_pre, real_pre, name)
    score_data['预测功率'] = res
    score_data.to_csv(r'E:\国能竞赛\dataset\score.csv', index=False)
    res_score = score.score(number, r'E:\国能竞赛\dataset\score.csv')
    log_in(name, res_score, bool)
...
def network_k_fold(train_data, test_data, name, number, parm_list, bool):
    ...
    X = train_data[train_data['白天'] == bool][
        ['辐照度','风速','风向','温度','湿度','压强','实际功率']]
    # y = train_data['实际功率']
    y = test_data[test_data['白天'] == bool][
        ['辐照度','风速','风向','温度','湿度','压强',]]

    train = X[['辐照度','风速','风向','温度','湿度','压强',]]
    targets = X[['实际功率']]
    test_data_y = y
    model = build_model(6)
    for i in range(k):
        ...
        注意： 默认为 1
        '''
        model.fit(partial_train_data, partial_train_targets, epochs=200, batch_size=1, verbose=1)
...
train_data_1.sort_values("时间", inplace=True)
################################测试集###########################
test_path_1 = r'E:\国能竞赛\dataset\new_dataset\test\test_1.csv'
test_file_list_1 = [test_path_1]
test_data_1 = pd.DataFrame(conact_data(test_file_list_1))
# 电站2
################################训练集###########################
path_2 = r'E:\国能竞赛\dataset\new_dataset\train\train_2.csv'
file_list_2 = [path_2]
train_data_2 = pd.DataFrame(conact_data(file_list_2))
train_data_2['时间'] = [change_time_stamp(i) for i in train_data_2['时间']]
train_data_2.sort_values("时间", inplace=True)
################################测试集###########################
test_path_2 = r'E:\国能竞赛\dataset\new_dataset\test\test_2.csv'
test_file_list_2 = [test_path_2]
test_data_2 = pd.DataFrame(conact_data(test_file_list_2))
# 电站3
################################训练集###########################
path_3 = r'E:\国能竞赛\dataset\new_dataset\train\train_3.csv'
file_list_3 = [path_3]
train_data_3 = pd.DataFrame(conact_data(file_list_3))
train_data_3['时间'] = [change_time_stamp(i) for i in train_data_3['时间']]
train_data_3.sort_values("时间", inplace=True)
################################测试集###########################
test_path_3 = r'E:\国能竞赛\dataset\new_dataset\test\test_3.csv'
test_file_list_3 = [test_path_3]
test_data_3 = pd.DataFrame(conact_data(test_file_list_3))
# 电站4
################################训练集###########################
path_4 = r'E:\国能竞赛\dataset\new_dataset\train\train_4.csv'
file_list_4 = [path_4]
train_data_4 = pd.DataFrame(conact_data(file_list_4))
train_data_4['时间'] = [change_time_stamp(i) for i in train_data_4['时间']]
train_data_4.sort_values("时间", inplace=True)
################################测试集###########################
test_path_4 = r'E:\国能竞赛\dataset\new_dataset\test\test_4.csv'
test_file_list_4 = [test_path_4]
test_data_4 = pd.DataFrame(conact_data(test_file_list_4))
# 电站5
################################训练集###########################
path_5 = r'E:\国能竞赛\dataset\new_dataset\train\train_5.csv'
file_list_5 = [path_5]
train_data_5 = pd.DataFrame(conact_data(file_list_5))
train_data_5['时间'] = [change_time_stamp(i) for i in train_data_5['时间']]
train_data_5.sort_values("时间", inplace=True)
################################测试集###########################
test_path_5 = r'E:\国能竞赛\dataset\new_dataset\test\test_5.csv'
test_file_list_5 = [test_path_5]
test_data_5 = pd.DataFrame(conact_data(test_file_list_5))
# 电站6
################################训练集###########################
path_6 = r'E:\国能竞赛\dataset\new_dataset\train\train_6.csv'
file_list_6 = [path_6]
train_data_6 = pd.DataFrame(conact_data(file_list_6))
train_data_6['时间'] = [change_time_stamp(i) for i in train_data_6['时间']]
train_data_6.sort_values("时间", inplace=True)
################################测试集###########################
test_path_6 = r'E:\国能竞赛\dataset\new_dataset\test\test_6.csv'
test_file_list_6 = [test_path_6]
test_data_6 = pd.DataFrame(conact_data(test_file_list_6))
# 电站7
################################训练集###########################
path_7 = r'E:\国能竞赛\dataset\new_dataset\train\train_7.csv'
file_list_7 = [path_7]
...
################################测试集###########################
test_path_9 = r'E:\国能竞赛\dataset\new_dataset\test\test_9.csv'
