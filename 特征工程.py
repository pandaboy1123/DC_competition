...
####################################################################################################
# 新特征组合开始
def new_poly_f(path,train_path):
    '''
    新构建的特征组合
    :param path:
    :param name:
    ...
    with open(path, 'r', encoding='utf-8') as f:
        data = pd.read_csv(f)
        poly_train = data[['总辐射', '直辐射', '散辐射', '辐照度', '风速', '风向','温度','湿度','压强',
                           '月_1','日_1','时_1','分_1']]
        poly_transformer = PolynomialFeatures(degree=3)
        poly_transformer.fit(poly_train)
        poly_train = poly_transformer.transform(poly_train)
        feature_name = poly_transformer.get_feature_names(input_features=['总辐射', '直辐射', '散辐射', '辐照度', '风速', '风向','温度','湿度','压强',
                           '月_1','日_1','时_1','分_1'])
        poly_features = pd.DataFrame(poly_train,
                                     columns=poly_transformer.get_feature_names(['总辐射', '直辐射', '散辐射', '辐照度', '风速', '风向','温度','湿度','压强',
                           '月_1','日_1','时_1','分_1']))
        name_li = feature_name[14:]
        data_need = poly_features[name_li]
        name_list = [name_li[i:i + 8] for i in range(0, len(name_li), 8)]
        col_li = []
        with open(train_path,'r',encoding='utf-8') as f:
            train_data = pd.read_csv(f)
        ...
        #     plt.rcParams['font.sans-serif'] = ['SimHei']  # 用来正常显示中文标签
        #     plt.rcParams['axes.unicode_minus'] = False  # 用来正常显示负号
        #     new_data = pd.concat([data['实际功率'], data_need[i]], axis=1)
        #     correlations = new_data.corr()
        #     correction = abs(correlations)
        #     for i in correction[correction['实际功率']>0.5]['实际功率']._stat_axis.values:
        #         col_li.append(i)
        # col_li = list(set(col_li))
        col_li.remove('实际功率')
        col_li.remove('实际辐照度')
        col_li.insert(0,'id')
        if 'Unnamed: 0' in col_li:
            col_li.remove('Unnamed: 0')
        else:
            pass
        new_data = pd.concat([data, data_need], axis=1)[col_li]
        new_data.to_csv(path, index=False)
        print(path+'保存已完成')
def opfile(test_path,train_path):
    with open(test_path,'r',encoding='utf-8') as f:
        print(test_path,len(pd.read_csv(f).columns.values))
    with open(train_path,'r',encoding='utf-8') as f:
        print(train_path,len(pd.read_csv(f).columns.values))
import os
path_list = []
train_path_list = []
...
for root, dirs, files in os.walk(r'E:\国能竞赛\dataset\new_dataset\train'):
    for file in files:
        train_path_list.append(os.path.join(root,file))
for i in range(len(path_list)):
    new_poly_f(path_list[i],train_path_list[i])
...
#     new_data = new_data[['时间', '总辐射' ,'直辐射' ,'散辐射', '辐照度', '风速', '风向' ,'温度' ,'湿度', '压强' ,'实际辐照度', '实际功率', '年',
#  '月' ,'日', '时', '分', '白天', '年_1', '月_1' ,'日_1' ,'时_1' ,'分_1']]
#     new_data.to_csv(real_path,index=False)