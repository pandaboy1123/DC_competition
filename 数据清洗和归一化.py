...
# power_9_path = r'E:\国能竞赛\dataset\new_dataset\气象数据\电站9_气象.csv'
# with open(power_9_path,'r',encoding='utf-8') as f1:
#     power_data = pd.read_csv(f1)
# train_9_path = r'E:\国能竞赛\demo\train___9.csv'
# with open(train_9_path,'r',encoding='utf-8') as f:
#     data = pd.read_csv(f)
#     new_data = pd.merge(data,power_data,how='left',on='时间')
#     new_data['时间戳'] = [change_time_stamp(i) for i in new_data['时间']]
#     new_data = new_data[new_data['时间戳']>1490111100]
#     new_data = new_data.drop(columns=['辐照度','风速_x','风向_x' ,'温度_x', '湿度_x', '压强_x','时间戳'],axis=1)
#     new_data.rename(columns = {'风速_y':'风速','风向_y':'风向','温度_y':'温度','湿度_y':'湿度','压强_y':'压强'},inplace=True)
#     print(new_data.columns.values, len(new_data))
#     new_data.to_csv('train___9.csv',index=False)

# train_10_path = r'E:\国能竞赛\dataset\new_dataset\train\train___10.csv'
# power_10_path = r'E:\国能竞赛\dataset\new_dataset\气象数据\电站10_气象.csv'
# with open(train_10_path,'r',encoding='utf-8') as f:
#     data = pd.read_csv(f)
# with open(power_10_path,'r',encoding='utf-8') as f1:
#     power_data = pd.read_csv(f1)
#     new_data = pd.merge(data,power_data,how='left',on='时间')
#     new_data['时间戳'] = [change_time_stamp(i) for i in data['时间']]
#     new_data = new_data[~(new_data['时间戳']>=1519919100)&(new_data['时间戳']<=1522856700)]
#     new_data = new_data.drop(columns=['辐照度', '风速_x', '风向_x', '温度_x', '湿度_x', '压强_x', '时间戳'], axis=1)
#     new_data.rename(columns={'风速_y': '风速', '风向_y': '风向', '温度_y': '温度', '湿度_y': '湿度', '压强_y': '压强'}, inplace=True)
#     new_data.to_csv('train___10.csv',index=False)

# path_1 = r'E:\国能竞赛\demo\train___9.csv'
# path_2 = r'E:\国能竞赛\demo\train___10.csv'
...
#     print(data.columns.values,len(data_2))
#     if data.columns.values.all() == data.columns.values.all():
#         print('True')
...
# path_10 = r'E:\国能竞赛\demo\test___10.csv'
